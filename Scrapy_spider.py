...
from ..items import OsrsExtractItem
import datetime
...
class ExtractionSpider(scrapy.Spider):
    ...
    ## Reads start_urls incrementally from csv file
    def start_requests(self):
        with open('/home/ec2-user/PROJECTFOLDER/item_links.csv') as f:
            for line in f:
                if len(line.strip('"')) == 0:
                    continue
    ...
    def parse(self, response, **kwargs):
        ...
        line_text = all_lines[ind]  ## Extract today's price data
        split = line_text.split(", ")
        price = ''.join(filter(str.isdigit, split[1]))
        price_list.append(int(price))
        ind = ind + 1

        line_text = all_lines[ind]  ## Extract today's volume data
        split = line_text.split(", ")
        volume = ''.join(filter(str.isdigit, split[1]))
        volume_list.append(int(volume))
        ind = ind + 1
        ...
        yield items                 ## Pass data through item pipeline


## Run command
## Scrapy crawl OSRSGE -o scrapy_output.csv